 ...
 * @param
 */
export function useFormCallback<TFormValues>({
  name,
  watch = false,
  clear = false,
}: FormInputBaseProps<TFormValues>) {
  ...
  useEffect(() => {
    if (watch) {
      trigger(name);
    } // trigger validation on change manually
    if (isDirty || isTouched) dismiss(); // dismiss messages from fetching data
  }, [watching, trigger, name, watch, dismiss, isDirty, isTouched]);

  useEffect(() => {
    if (clear && isSubmitSuccessful && (!message || message.type === "success"))
      resetField(name); // manually reset field after a successful submit
